...
1. **What if the user presents a QR Token printed on paper?**
    ...
    In cases where the user is holding an offline image of the Token's QR (e.g., printed or screenshot), [Consumer 💼 domains](<../../25 Data/27 💼 Consumers/04 💼🎭 Consumer role.md>) may ask for a synchronous status check of the [Token 🎫](<01 🎫 Token.md>) directly with the [user's Broker 🤵 domain](<../03 🤵 Brokers/03 🤵 Broker domain.md>);
    
    - e.g., while validating a printed QR of a [passport Token 🎫](<01 🎫 Token.md>) at the airport border control, the [Consumer 💼 domain](<../../25 Data/27 💼 Consumers/04 💼🎭 Consumer role.md>) of the airport may try to reach the [user's Broker 🤵 domain](<../03 🤵 Brokers/03 🤵 Broker domain.md>) to verify if the passport hasn't been suspended or revoked.
