...
1. **What is a Notifier domain in NLWeb?**
    ...
    * This allows [Broker 🤵 domains](<../03 🤵 Brokers/$ 🤵 Broker domain.md>) to communicate in standard HTTPS with the [Notifier 📣 domain](<$ 📣 Notifier domain.md>), while the [Notifier 📣 domain](<$ 📣 Notifier domain.md>) communicates via mobile-friendly real-time protocols with the [Wallet 🧑‍🦰 app](<../01 🧑‍🦰 Wallets/$ 🧑‍🦰 Wallet app.md>) (e.g., MQTT, web sockets, mobile notifications). 
...
1. **Do Notifier domains store user data?**
    ...
    * In NLWeb, domain orchestration is done by [Broker 🤵 domains](<../03 🤵 Brokers/$ 🤵 Broker domain.md>), and storage of user data is done by [Vault 🗄️](<../../41 🎭 Domain Roles/80 🗄️ Vaults/$ 🗄️🎭 Vault role.md>) domains, while a [Notifier 📣 domain's](<$ 📣 Notifier domain.md>) only responsibility is to proxy [domain messages 📨](<../../40 👥 Domains/41 📨 Messages/01 📨 Domain Message.md>) to and from the [Broker 🤵 domain](<../03 🤵 Brokers/$ 🤵 Broker domain.md>). 
    
    * This is by design, allowing resilient [Broker 🤵 domain](<../03 🤵 Brokers/$ 🤵 Broker domain.md>) implementations (e.g., AWS, GCP, Azure) to assume the complexity of the undifferentiated NLWeb protocol, while allowing app start-ups to focus on user experience differentiation. 
    
    * Of course, companies developing [Wallet 🧑‍🦰 apps](<../01 🧑‍🦰 Wallets/$ 🧑‍🦰 Wallet app.md>) may wish to support unique features not supported by [Broker 🤵 domain](<../03 🤵 Brokers/$ 🤵 Broker domain.md>), which may eventually result in the need to store user data on the [Notifier 📣 domain](<$ 📣 Notifier domain.md>).
