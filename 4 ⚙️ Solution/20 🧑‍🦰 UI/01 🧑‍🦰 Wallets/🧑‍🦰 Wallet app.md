...
1. **What data exists in the device at any given time?**
    ...
    - 📣 The Wallet ID provided by the [Broker 🤵 domain](<../../45 🤲 Helper domains/24 🤵 Brokers/$ 🤵 Broker domain.md>) via the [Notifier 📣 domain](<../02 📣 Notifiers/📣 Notifier domain.md>);
...
1. **How do users migrate a Wallet to another phone?**
    ...
    * The [Broker 🤵 domain](<../../45 🤲 Helper domains/24 🤵 Brokers/$ 🤵 Broker domain.md>) will invoke the [Identity 🆔 domain](<../../50 🫥 Agents/45 🆔 Identities/$ 🆔🫥 Identity agent.md>) on the new phone to perform an identity authentication (e.g., face scan), and then will automatically decommission the old Wallet.
...
1. **How do users change between Wallet providers?**
    ...
    If both the old and the new [Wallet 🧑‍🦰 app](<🧑‍🦰 Wallet app.md>) providers use the same [Broker 🤵 domain](<../../45 🤲 Helper domains/24 🤵 Brokers/$ 🤵 Broker domain.md>), 
    * then changing between Wallet providers in the same phone is very similar to migrating a Wallet to another phone. 
    
    If they use different [Broker 🤵 domains](<../../45 🤲 Helper domains/24 🤵 Brokers/$ 🤵 Broker domain.md>), 
    * then these Brokers will need to implement some sort of portability. 
     
    For simplicity, let's assume they use the same [Broker 🤵 domain](<../../45 🤲 Helper domains/24 🤵 Brokers/$ 🤵 Broker domain.md>).
...
1. **What if an attacker intercepts a user's recovery QR Token?**
    ...
    When a migration QR is used on a new [Wallet 🧑‍🦰 app](<🧑‍🦰 Wallet app.md>), the [Broker 🤵 domain](<../../45 🤲 Helper domains/24 🤵 Brokers/$ 🤵 Broker domain.md>) notifies the old Wallet, allowing legitimate owners to block the attack and destroy the QR. 
    
    * For situations where legitimate owners are not aware of notifications, migrations have a small grace period where [Broker 🤵 domains](<../../45 🤲 Helper domains/24 🤵 Brokers/$ 🤵 Broker domain.md>) inactivate both Wallets until the old Wallet accepts the transfer or the grace period expires.
...
1. **What workflows can users execute in a Wallet app?**
    ...
    |-|-|-
    | 🤵 Set up | [👉 Onboard](<../../../5 ⏩ Flows/90 🧑‍🦰👉 Wallets/10 👉🤵 Set-up/11 🧑‍🦰👉🤵 Onboard.md>)  | Register the [Wallet 🧑‍🦰](<🧑‍🦰 Wallet app.md>) on a [Broker 🤵](<../../45 🤲 Helper domains/24 🤵 Brokers/$ 🤵 Broker domain.md>)
    ...
    ||[👉 Printer QR](<../../../5 ⏩ Flows/90 🧑‍🦰👉 Wallets/15 👉🔆 Locators/02 🧑‍🦰👉🤗 Scan printer QR.md>)  | Scan a [Printer 🖨️ NFC/QR](<../../45 🤲 Helper domains/75 🖨️ Printers/🖨️🛠️ Printer helper.md>) to open a [Chat 💬](<../12 💬 Chats/$ 💬 Chat.md>)
