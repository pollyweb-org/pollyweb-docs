...
1. **How do users change between Wallet providers?**
    ...
    If both the old and the new [Wallet 🧑‍🦰 app](<🧑‍🦰 Wallet app.md>) providers use the same [Broker 🤵 domain](<../../45 🛠️ Helper domains/24 🤵 Brokers/$ 🤵 Broker domain.md>), 
...
1. **What if an attacker intercepts a user's recovery QR Token?**
    ...
    When a migration QR is used on a new [Wallet 🧑‍🦰 app](<🧑‍🦰 Wallet app.md>), the [Broker 🤵 domain](<../../45 🛠️ Helper domains/24 🤵 Brokers/$ 🤵 Broker domain.md>) notifies the old Wallet, allowing legitimate owners to block the attack and destroy the QR. 
