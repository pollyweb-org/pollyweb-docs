...
1. **What is a chat?**
    ...
    * between a [Wallet 🧑‍🦰 app](<../01 🧑‍🦰 Wallets/$ 🧑‍🦰 Wallet app.md>) and a [Host 🤗 domain](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) 
...
1. **How are the intervening parties in a chat?**
    ...
    |-|-
    |[🤗 Host](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) | Leads the [Chat 💬](<$ 💬 Chat.md>), always asking first.
    |[🧑‍🦰 Wallet](<../01 🧑‍🦰 Wallets/$ 🧑‍🦰 Wallet app.md>) | Held by the user, replies directly to the [Host 🤗 domain](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>).
    |[🤵 Broker](<../03 🤵 Brokers/$ 🤵 Broker domain.md>) | Intermediates the conversation, receiving the Host's messages and forwarding them to the [Notifier 📣 domain](<../02 📣 Notifiers/$ 📣 Notifier domain.md>).
    |[📣 Notifier](<../02 📣 Notifiers/$ 📣 Notifier domain.md>) | Implements the mobile push notifications, receiving the messages from the [Broker 🤵 domain](<../03 🤵 Brokers/$ 🤵 Broker domain.md>) and forwarding them to the [Wallet 🧑‍🦰 app](<../01 🧑‍🦰 Wallets/$ 🧑‍🦰 Wallet app.md>).
...
1. **How does a user open a new chat window with a domain?**
    ...
    To open a new [Chat 💬](<$ 💬 Chat.md>) window with a [Host 🤗 domain](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>), a user needs the [Host's Locator 🔆](<../11 🔆 Locators/$ 🔆 Locator.md>) contained in one of the following technology forms.
    ...
    |[🤔 Prompt](<../../../9 😃 Talkers/10 📘 Talker specs/20 🤔 Prompt.md>)| In a [Chat 💬](<$ 💬 Chat.md>) window, the [Host 🤗 domain](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) provides one or more options for the user to select from, and the user selects one - this opens a new [Chat 💬](<$ 💬 Chat.md>) window.
...
1. **What are the message types supported in chats?**
    ...
    | Component | Behavior
    |-|-
    | [🤔 Prompt](<../../../9 😃 Talkers/10 📘 Talker specs/20 🤔 Prompt.md>) | [Host 🤗 domains](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) send a question or information to the user.
...
1. **How can Hosts leverage reference data, like countries?**
    ...
    [Hosts 🤗 domains](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) can use data sets exposed by [Dataset 🪣 helper domains](<07 🪣🎭 Dataset role.md>).
...
1. **How can users contribute to a domain's reputation?**
    ...
    * [Host 🤗 domain](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) managers are notified about the feedback, and can reply to them. Similar mechanisms have been widely adopted by Trust Pilot, Google Maps, and Google Play.
...
1. **Do chats support map navigation?**
    ...
    No. 
    - [Host 🤗 domains](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) can send location pins in user chats, then users can click on them to open the device's default map navigation app.
