...
1. **What is a Broker domain in NLWeb?**
    ...
    A Broker is any [domain 👥](<../../40 👥 Domains/44 📜 Manifests/00 👥 Domain.md>) that orchestrates the relationship between [Wallet 🧑‍🦰 apps](<../01 🧑‍🦰 Wallets/01 🧑‍🦰 Wallet app.md>) and [Host 🤗 domains](<../23 💬 Chats/03 🤗🎭 Host role.md>), working as the [Wallet's 🧑‍🦰](<../01 🧑‍🦰 Wallets/01 🧑‍🦰 Wallet app.md>) database.
...
1. **How do Brokers protect users from Hosts?**
    ...
    * For a [Host 🤗 domain](<../23 💬 Chats/03 🤗🎭 Host role.md>) to identity a user across sessions, the user needs to explicitly accept a [Bind 🔗](<../24 🗄️ Vaults/01 🔗 Bind.md>) or [Token 🎫](<../25 🎫 Tokens/01 🎫 Token.md>) offered by the [Host 🤗 domain](<../23 💬 Chats/03 🤗🎭 Host role.md>).
...
3. **What if an attacker intercepts a user's recovery QR Token?**
...
4. **After destroying a migration QR, how can users migrate?**

    Just generate a new migration QR.

    ---

5. **After losing a phone, how do users recover a wallet on a new phone?**
...
6. **What if an attacker has the user's old phone and rejects the transfer?**
...
7. **Why aren't Brokers and Notifiers the same domain?**
...
8. **How can Wallet startups connect to a Broker?**
...
9. **How do Brokers ensure Wallets are NLWeb compliant?**
