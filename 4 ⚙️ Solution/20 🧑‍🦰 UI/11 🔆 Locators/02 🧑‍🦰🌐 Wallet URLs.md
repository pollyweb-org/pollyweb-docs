...
1. **What if an NLWeb Wallet is not installed in the device?**
    ...
    NLWeb advocates for developers to use the redirect script at `https://nlweb.org/go/` so that users without an installed [Wallet 🧑‍🦰 app](<../01 🧑‍🦰 Wallets/$ 🧑‍🦰 Wallet app.md>) can fallback to a page with instructions on how to install one. 
...
1. **How can website developers add NLWeb links?**
    ...
    * Website developers can also opt to link the JavaScript library `https://nlweb.org/sdk/nlweb.js` - this allows [Wallets 🧑‍🦰](<../01 🧑‍🦰 Wallets/$ 🧑‍🦰 Wallet app.md>) to achieve the same behavior without browser redirects.
...
1. **What happens to NFC/QR Locators if `nlweb.org` goes down?**
    ...
    * Users using their [Wallet 🧑‍🦰 apps](<../01 🧑‍🦰 Wallets/$ 🧑‍🦰 Wallet app.md>) as the default NFC/QR reader won't notice any difference, because the [Wallet 🧑‍🦰](<../01 🧑‍🦰 Wallets/$ 🧑‍🦰 Wallet app.md>) will parse the destination [Host 🤗](<../12 💬 Chats/04 🤗🎭 Host role.md>) from the URL.
