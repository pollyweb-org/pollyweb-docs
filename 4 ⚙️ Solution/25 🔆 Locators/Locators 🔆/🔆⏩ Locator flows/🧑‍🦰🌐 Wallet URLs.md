...
1. **How can app developers handle NLWeb URLs?**
    ...
    An NLWeb [Locator 🔆](<../🔆 Locator.md>) link is an Uniform Resource Locator (URL) starting with the custom protocol `pollyweb://` (e.g., a locator `ABC` can be handled with the URL `pollyweb://ABC`). 
...
1. **How can website developers add NLWeb links?**
    ...
    * If no mobile app opens the `pollyweb://` URL, then the proxy redirects to a fallback webpage explaining to the user how to find and install an app that supports the NLWeb protocol. 
