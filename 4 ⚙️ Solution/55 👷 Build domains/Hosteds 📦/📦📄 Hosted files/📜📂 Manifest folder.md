# 📜 Hosted `Manifest` folder

> Part of [Hosted 📦 domain](<../📦👥 Hosted domain.md>)

<br/>


1. **What is the Manifest folder?**

    The `📜 Manifest/` folder 
    * contains the [domain Manifest 📜](<../../../30 🧩 Data/Manifests 📜/📜 Manifest/📜 Manifest.md>) parts
    * broken into multiple files and folders.
    
    ---
    <br/>

1. **What are the advantages of multiple files?**

    Advantage | Details
    |-|-
    |`Maintenance` | Multiple files change in a granular fashion.
    |`Resilience` | If one file has an error, the others still work.
    |`Size limits`| Independent parts don't hit cloud size limits. 
    

    ---
    <br/>

1. **What is the structure of a Manifest?**

    * `📜 Manifest/` 
        * [`👥 About.yaml`](<../../../45 🤲 Helper domains/Graphs 🕸/🕸📨 Graph msgs/👥🚀🕸 About/🕸 About 🚀 call.md>) 
        * `📝 Forms/`   
            * [`📝 FORM-1.yaml`](<../../../45 🤲 Helper domains/Graphs 🕸/🕸📨 Graph msgs/👥🚀🕸 Form/🕸 Form 🚀 call.md>)
        * `🧩 Codes/` 
            * [`🧩 CODE-1.yaml`](<../../../45 🤲 Helper domains/Graphs 🕸/🕸📨 Graph msgs/👥🚀🕸 Schema/🕸 Schema 🚀 call.md>)
            * `GROUP-A/`
                * [`🧩 CODE-A2.yaml`](<../../../45 🤲 Helper domains/Graphs 🕸/🕸📨 Graph msgs/👥🚀🕸 Schema/🕸 Schema 🚀 call.md>)
        * `🫡 Trusts/` 
            * [`🫡 TRUST-1.yaml`](<../../../30 🧩 Data/Trusts 🫡/🫡 Domain Trust.md>) 
            * `GROUP-C/`
                * [`🫡 TRUST-C2.yaml`](<../../../30 🧩 Data/Trusts 🫡/🫡 Domain Trust.md>) 
        * `🪢 Services/` 
            * `🪢 INTEGRATION-1.yaml`
            * `GROUP-S/`
                * `🪢 INTEGRATION-S2.yaml`

    ---
    <br/>