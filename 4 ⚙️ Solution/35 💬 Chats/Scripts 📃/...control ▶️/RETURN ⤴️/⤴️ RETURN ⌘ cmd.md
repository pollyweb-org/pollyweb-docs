...
1. **What's a RETURN command?**
    ...
    * and returns one of the following expressions.
    ...
    |-|-
    |`<string>`| `3` `Alice`
    [`{Function}`](<../../../Talkers 😃/😃⚙️ Talker cmds/...functions 🐍/{Function} 🐍.md>) | `{$holder}` `{handler(1)}` `{.helper(1)}` 
...
1. **What's the RETURN syntax?**
    ...
    |-|-|-
    | `{expression}`| String or [{Function}](<../../../Talkers 😃/😃⚙️ Talker cmds/...functions 🐍/{Function} 🐍.md>) to be evaluated | `A` `{f}` `{$p}`
...
1. **How to read the result from a RETURN?**
    ...
    Commands: [`.Add`](<../../../Talkers 😃/😃⚙️ Talker cmds/...functions 🐍/🔩 {.Add}.md>) [`INFO`](<../../../Prompts 🤔/🤔📢 Prompt status/INFO ℹ️/INFO ℹ️ prompt.md>) [`RUN`](<../RUN ▶️/▶️ RUN ⌘ cmd.md>) 
