...
1. **What's an IF flow?**
    ...
    * based on the evaluation of a holder or [{Function}](<../../📃 basics/Function 🐍.md>).
...
1. **What's the IF syntax for then-only IFs?**
    ...
    |-|-|-
    | `{function}` | Boolean [{Function}](<../../📃 basics/Function 🐍.md>) to evaluate  | `{f}` `{$o}`
...
1. **What's the IF syntax for multi-line command lists?**
    ...
    | Input| Purpose
    |-|-
    | `<true-cmd-n>` | List of multi-line [Commands ⌘](<../../📃 basics/Command ⌘.md>) to run on `True`
    | `<false-cmd-n>` | List of multi-line [Commands ⌘](<../../📃 basics/Command ⌘.md>) to run on `False`
