...
1. **What's an IF flow?**
    ...
    * that runs a [Procedure ⚙️](<⚙️ Procedure.md>) or [Command ⌘](<⌘ Command.md>) 
    * based on the evaluation of a placeholder or [{Function}](<../😃💾 Talker data/{Function} 🐍.md>).
...
1. **How are values evaluated to booleans?**
    ...
    [Share 💼](<../😃📨 Talker msgs/SHARE 💼 msg.md>) | empty | `{}`| non-empty | `{A:1}`
...
1. **What's the `IF` syntax?**
    ...
    |-|-|-
    | `{function}` | Boolean [{Function}](<../😃💾 Talker data/{Function} 🐍.md>) to evaluate  | `{f}` `{$o}`
    ...
    | Argument| Purpose
    |-|-
    | `<true-action>` | [Procedure ⚙️](<⚙️ Procedure.md>) or one-line [Command ⌘](<⌘ Command.md>) on `True`
    | `<false-action>`| [Procedure ⚙️](<⚙️ Procedure.md>) or one-line [Command ⌘](<⌘ Command.md>) on `False`
