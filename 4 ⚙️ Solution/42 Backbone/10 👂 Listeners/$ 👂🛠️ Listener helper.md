...
1. **How do Listeners work?**
    ...
    |#|Step
    |-|-
    |1| [Domains 👥](<../../40 👥 Domains/41 📨 Messages/00 👥 Domain.md>) publish their [Manifest 📜](<../../40 👥 Domains/44 📜 Manifests/01 📜 Domain Manifest.md>) updates to the DNS endpoint of the global Listeners 👂 cluster. As a fallback, the same updates may be sent to an individual [Listener 👂 domain](<$ 👂🛠️ Listener helper.md>).
    ...
    |4| Each [Listener 👂 domain](<$ 👂🛠️ Listener helper.md>) performs a fan-out propagation of update events to all [Graph 🕸 helper domains](<../20 🕸 Graphs/$ 🕸🛠️ Graph helper.md>) that [subscribed 🔔](<../../40 👥 Domains/42 🌬️ Streams/04 🔔🎭 Subscriber role.md>) to the Listener's 👂 [domain-event Stream 🌬️](<../../40 👥 Domains/42 🌬️ Streams/02 🌬️🎭 Streamer role.md>).
...
1. **Why are Listeners necessary?**
    ...
    * [Listener 👂 domains](<$ 👂🛠️ Listener helper.md>) ensure that the cache in these [Graph 🕸 domains](<../20 🕸 Graphs/$ 🕸🛠️ Graph helper.md>) is updated in near-real-time by propagating domain update notifications to [Graph 🕸 domains](<../20 🕸 Graphs/$ 🕸🛠️ Graph helper.md>). 
...
1. **How aren't Listeners and Graphs the same?**
    ...
    * While [Graph 🕸 domains](<../20 🕸 Graphs/$ 🕸🛠️ Graph helper.md>) can be built by anyone, [Listener 👂 domains](<$ 👂🛠️ Listener helper.md>) are a lightweight layer managed by a coordinated consortium of cloud providers.
...
1. **Do Listeners read domain Manifests from the domain?**
    ...
    * However, [domains 👥](<../../40 👥 Domains/41 📨 Messages/00 👥 Domain.md>) may explicitly request [Listener 👂 domains](<$ 👂🛠️ Listener helper.md>) to reset the domain's [Manifest 📜](<../../40 👥 Domains/44 📜 Manifests/01 📜 Domain Manifest.md>) based on a content located in a given URL, as long as the content doesn't reach a maximum size for a [Manifest 📜](<../../40 👥 Domains/44 📜 Manifests/01 📜 Domain Manifest.md>).
...
1. **Why is there a size limitation on the entire Manifest?**
    ...
    Domains may ask [Graph 🕸 domains](<../20 🕸 Graphs/$ 🕸🛠️ Graph helper.md>) and [Listener 👂 domains](<$ 👂🛠️ Listener helper.md>) to download their [Manifest 📜](<../../40 👥 Domains/44 📜 Manifests/01 📜 Domain Manifest.md>) for drift detection and sync reset. 
...
1. **What's the retention commitment of Listeners?**
    ...
    [Listener 👂 domains](<$ 👂🛠️ Listener helper.md>) keep all changes from all domain [Manifests 📜](<../../40 👥 Domains/44 📜 Manifests/01 📜 Domain Manifest.md>) and public keys indefinitely. 
...
1. **What if a subscriber wants to read all history?**
    ...
    [Subscriber 🔔 domains](<../../40 👥 Domains/42 🌬️ Streams/04 🔔🎭 Subscriber role.md>) can ask [Listener 👂 domains](<$ 👂🛠️ Listener helper.md>) to replay all domain updates in a given period, or from the beginning of times.
...
1. **Is there data loss when a Listener goes down?**
    ...
    No. 
    * Each [Listener 👂 domain](<$ 👂🛠️ Listener helper.md>) is fully independent, being responsible for replying the entire history of [domain Manifest 📜](<../../40 👥 Domains/44 📜 Manifests/01 📜 Domain Manifest.md>) changes, even if it is the only [Listener 👂 node](<$ 👂🛠️ Listener helper.md>) available in the cluster. 
...
1. **How can a subscriber filter notifications by content?**
    ...
    [Subscriber 🔔 domains](<../../40 👥 Domains/42 🌬️ Streams/04 🔔🎭 Subscriber role.md>) can set a filter when subscribing to [Listener 👂 domains](<$ 👂🛠️ Listener helper.md>):
