💍 Userables to pay a salesperson
===

> [Userables 💍](<01 💍 Userable thing.md>) allow customers to pay without battery on their devices.

> Mentioned in [Verify Userables 🆔](<../../30 🫥 Agents/45 🆔 Identities/15 🆔💍 Verify Userables.md>)

<br/>


1. **How can users pay to a salesperson with Userables?**



    [Custodians 🧳](<../71 💠 Brand Things/05 🧳🗄️ Custodian vault.md>) integrate with [💳 Payers](<../../30 🫥 Agents/60 💳 Payers/03 💳🎭 Payer role.md>) to allow users to pay with their [Userables 💍](<01 💍 Userable thing.md>) after being authenticated by their [Identity 🆔](<../../30 🫥 Agents/45 🆔 Identities/01 🆔🫥 Identity agent.md>) (e.g., face scan). 
    
    * This allows sales people to use their own [Wallets 🧑‍🦰](<../../20 🧑‍🦰 UI/01 🧑‍🦰 Wallets/$ 🧑‍🦰 Wallet app.md>) as a chat interface for the user. 

    ---
    <br/>


1. **How does it work?**

    ![](<00 📎 Assets/💍 Userable Pay Wallet.png>)

    ---
    <br/>
