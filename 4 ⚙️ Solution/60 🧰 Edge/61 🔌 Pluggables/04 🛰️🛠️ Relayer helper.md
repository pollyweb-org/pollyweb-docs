...
1. **What is a Relayer domains?**
    ...
    * owned by other [domains 👥](<../../40 👥 Domains/41 📨 Msgs/00 👥 Domain.md>).
...
1. **Why are Relayer domains important?**
    ...
    | Feature | Description
    |-|-
    | `Connectivity` | Relayer 🛰️ helpers ensure that there is a bidirectional real-time communication over the internet between an on-premise [Pluggable 🔌 device](<01 🔌 Pluggable device.md>) and the [domain 👥](<../../40 👥 Domains/41 📨 Msgs/00 👥 Domain.md>) who owns the [Antenna 📡 router device](<02 📡🔀 Antenna router.md>), thus removing the undifferentiated heavy-lifting of managing Wi-Fi onboarding and real-time communication with MQTT or WebSockets.
...
1. **How do Relayers work?**
    ...
    |5| `Graph`|  The Relayer 🛰️ domain reads the [API Schema 🧩](<../../20 🧑‍🦰 UI/24 🗄️ Vaults/02 🧩 Schema Code.md>) from a [Graph 🕸 helper domain](<../../40 👥 Domains/45 👂 Backbone/03 🕸🛠️ Graph helper.md>).
    |6| `Domain` | The Relayer 🛰️ domain informs the owner [domain 👥](<../../40 👥 Domains/41 📨 Msgs/00 👥 Domain.md>) of the newly-plugged [Pluggable 🔌 device](<01 🔌 Pluggable device.md>).
    |A| `Command`| The owner [domain 👥](<../../40 👥 Domains/41 📨 Msgs/00 👥 Domain.md>) sends a natural language command (e.g., `Test`) on the [Pluggable 🔌 device](<01 🔌 Pluggable device.md>).
    ...
    |Z| `Propagate` | The Relayer 🛰️ sends sends the translated events to the [Buffer ⏳ helper](<../../40 👥 Domains/42 🌬️ Streams/03 ⏳🛠️ Buffer helper.md>) of the owner [domain 👥](<../../40 👥 Domains/41 📨 Msgs/00 👥 Domain.md>).
...
1. **How do domains send commands to Pluggables?**
    ...
    For a [domain 👥](<../../40 👥 Domains/41 📨 Msgs/00 👥 Domain.md>) to send an API command to a [Pluggable 🔌 device](<01 🔌 Pluggable device.md>) via a Relayer 🛰️ domain, it needs to send the following parameters.
