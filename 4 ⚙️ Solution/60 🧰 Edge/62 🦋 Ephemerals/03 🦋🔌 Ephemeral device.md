...
1. **How is an Ephemeral applicable to the financial industry?**
    ...
    - The setup requires an [Antenna 📡 router](<../61 🔌 Pluggables/02 📡🔀 Antenna router.md>) on site, connected to the [Relayer 🛰️ domain](<../../45 🛠️ Helper domains/70 🛰️ Relayers/$ 🛰️🛠️ Relayer helper.md>), to allow the ATM [Host 🤗 domain](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) to remotely control the Ephemeral 🦋 device.
...
1. **How is an Ephemeral applicable to building security?**
    ...
    When opening a door with restricted access, attackers may put the NFC/QR [Locator 🔆](<../../20 🧑‍🦰 UI/11 🔆 Locators/$ 🔆 Locator.md>) of a high-security door A on a low-security door B on the other side of the building. 
    
    -   When a high-level user tries to open door B, they will actually be opening door A for the attacker. 
    - An Ephemeral 🦋 prevents this by periodically changing the [Locator 🔆](<../../20 🧑‍🦰 UI/11 🔆 Locators/$ 🔆 Locator.md>).
