...
1. **What are the possible statuses?**
    ...
    | `BLOCKED` | There is already an item with the same key and a different content, and the table schema was configured with `NoUpdates` to block any changes after the first [`Save@Itemizer` 📨 msg](<🛢 Save 🚀 call.md>).
    | `OUTDATED`  | The `.Version` of the item saved in the dataset (let's call it `A`) is different from the one given in `Item.Version` (let's call it B), meaning that item `A` has changed since item `B` was pulled with the [`Read@Itemizer` 📨 msg](<../Item Read 👥🚀🛢/🛢 Read 🚀 call.md>) call.
