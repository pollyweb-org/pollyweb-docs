...
1. **What is a Talker $placeholder?**
    ...
    * and managed by a [Talker 😃](<../10 😃 Talker.md>).
...
1. **How to read a $placeholder?**
    ...
    In a [Talker 😃](<../10 😃 Talker.md>) script,
...
1. **How to read properties from $placeholder objects?**
    ...
    In a [Talker 😃](<../10 😃 Talker.md>) script, use `dot` notation to access properties:
...
1. **How to write to a placeholder?**
    ...
    * `.Placeholders.Set('$p', new_value)` 
    ...
    In a [Talker 😃](<../10 😃 Talker.md>) script, use `>>` to send a value to a $placeholder:
    * `TEXT|bla >> $p` writes the answer to a [`TEXT` 🔠 input](<../../🤔 Prompts/7 ✏️ Input prompts/32 🔠 TEXT prompt.md>).
