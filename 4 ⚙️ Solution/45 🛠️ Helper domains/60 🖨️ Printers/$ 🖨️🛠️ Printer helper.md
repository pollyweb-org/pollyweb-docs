...
1. **What is a Printer domain in NLWeb?**
    ...
    * on behalf of [Brand 🍏 domains](<../../70 🌳 Ambient/71 💠 Brand Things/07 🍏🎭 Brand role.md>) or any other [Host 🤗 domain](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>).
...
1. **Why are Printers important?**
    ...
    * e.g., before going on trip through Asia, a user can buy a smart sticker in a supermarket then stick it on their old-yet-cherished notebook of memories so that anyone can return it free of charge if left behind.
    ...
    For businesses:
    * Printer 🖨️ domains offload from [Host 🤗 domains](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) the undifferentiated task of printing and distributing [NFC/QR Locators 🔆](<../../20 🧑‍🦰 UI/11 🔆 Locators/$ 🔆 Locator.md>). 
...
1. **How do Printers monetize?**
    ...
    - **Supplier sell**: Printers 🖨️ may supply [Brand 🍏 domains](<../../70 🌳 Ambient/71 💠 Brand Things/07 🍏🎭 Brand role.md>) and other [Host 🤗 domains](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) with printing [Locators 🔆](<../../20 🧑‍🦰 UI/11 🔆 Locators/$ 🔆 Locator.md>) that these domains then integrate into their products.
...
1. **Do Hosts have to know the Locators in advance?**
    ...
    - **with provided Locators**: 
        - the [Host 🤗 domain](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) sends a supply order with the [Locators 🔆](<../../20 🧑‍🦰 UI/11 🔆 Locators/$ 🔆 Locator.md>) to be printed, the quantity, and the format;
        - these are preferred where the [Locator 🔆](<../../20 🧑‍🦰 UI/11 🔆 Locators/$ 🔆 Locator.md>) is customized for a certain [Host 🤗](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) resource 
        - e.g., `any-supermarket.com/london-store` to reference a specific store in a chain of supermarkets.

    - **with anonymous Locators**: 
        - the [Host 🤗 domain](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) sends a supply order just specifying the quantity, the format, and the commitment lifetime;
        - these are preferred where the [Host 🤗 domain](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) wants to bind the [Locator 🔆](<../../20 🧑‍🦰 UI/11 🔆 Locators/$ 🔆 Locator.md>) at the final manufacturing stage, saving the logistical challenge of matching [Locators 🔆](<../../20 🧑‍🦰 UI/11 🔆 Locators/$ 🔆 Locator.md>) to the right product through the assembly process.
...
9.  **How are anonymous Locators translated?**
    ...
    - For this, Printers charge [Host 🤗 domains](<../../41 🎭 Domain Roles/30 🤗 Hosts/04 🤗🎭 Host role.md>) for a commitment lifetime.
