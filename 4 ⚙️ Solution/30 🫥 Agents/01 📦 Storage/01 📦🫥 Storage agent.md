...
1. **Can users change to a different Storage domain?**
    ...
    - Failure to comply causes [Firewalls 🔥](<../../40 👥 Domains/43 👍 Trusts/03 🔥🛠️ Firewall helper.md>) to block the domain.
...
1. **How to implement a Storage vault on AWS?**
    ...
    - 📨 **Inbox**: a combination of the Distributer plus the Endpoint for domain [📨 Messaging](<../../40 👥 Domains/41 📨 Msgs/01 📨 Domain Message.md>).
...
1. **How to implement a Storage client cache on AWS?**
    ...
    Storage clients rely on the following components for domain [📨 Messaging](<../../40 👥 Domains/41 📨 Msgs/01 📨 Domain Message.md>):
