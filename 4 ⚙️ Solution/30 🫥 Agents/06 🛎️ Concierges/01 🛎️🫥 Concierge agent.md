...
1. **What are Concierge domains in NLWeb?**
    ...
    * allowing [Host 🤗 domains](<../../20 🧑‍🦰 UI/23 💬 Chats/04 🤗🎭 Host role.md>) to add tasks to the user's queue without a clear definition of how those tasks will be executed.
...
2. **What are examples for Concierge usages in NLWeb?**
    ...
    | [Pizzeria 🍕](<../../../3 🤝 Use Cases/02 🍽️ Eat & Drink/04 🍕 Order pizza/01 🍕 Customer @ Home 🏠/21 🏠 Order pizza.md>) |Find a courier for delivery.
...
5. **How do Concierges handle inbound tasks?**
    ...
    | #| Category|Step
    |-|-|-
    |1| `Chat`| On [Chat 💬](<../../20 🧑‍🦰 UI/23 💬 Chats/01 💬 Chat.md>) with a [Host 🤗 domain](<../../20 🧑‍🦰 UI/23 💬 Chats/04 🤗🎭 Host role.md>), the user asks for a task that requires the help of a [Supplier 🏭 domain](<02 🏭🎭 Supplier role.md>) (e.g., deliver a pizza).
    |2| `Task` | The [Host 🤗 domain](<../../20 🧑‍🦰 UI/23 💬 Chats/04 🤗🎭 Host role.md>) invokes the user's [Concierge 🛎️ agent](<01 🛎️🫥 Concierge agent.md>) to handle the task (e.g., find a courier to deliver the pizza).
