...
1. **How do Listeners work?**
    ...
    |#|Step
    |-|-
    |1| [Domains 👥](<../41 📨 Msgs/00 👥 Domain.md>) publish their [Manifest 📜](<01 📜 Domain Manifest.md>) updates to the DNS endpoint of the global Listeners 👂 cluster. As a fallback, the same updates may be sent to an individual [Listener 👂 domain](<02 👂🛠️ Listener helper.md>).
...
1. **What is contained in a Manifest-changed event?**
    ...
    An event from a [domain 👥](<../41 📨 Msgs/00 👥 Domain.md>) about a [Manifest 📜](<01 📜 Domain Manifest.md>) change comes inside an [envelope 📨](<../41 📨 Msgs/01 📨 Domain Message.md>) containing the following properties.
...
1. **Do Listeners read domain Manifests from the domain?**
    ...
    * However, [domains 👥](<../41 📨 Msgs/00 👥 Domain.md>) may explicitly request [Listener 👂 domains](<02 👂🛠️ Listener helper.md>) to reset the domain's [Manifest 📜](<01 📜 Domain Manifest.md>) based on a content located in a given URL, as long as the content doesn't reach a maximum size for a [Manifest 📜](<01 📜 Domain Manifest.md>).
