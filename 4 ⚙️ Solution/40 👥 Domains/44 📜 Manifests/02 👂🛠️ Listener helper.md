...
1. **How do Listeners work?**
    ...
    |#|Step
    |-|-
    |1| [Domains 👥](<../41 📨 Comms/00 👥 Domain.md>) publish their [Manifest 📜](<01 📜 Domain Manifest.md>) updates to the DNS endpoint of the global Listeners 👂 cluster. As a fallback, the same updates may be sent to an individual [Listener 👂 domain](<02 👂🛠️ Listener helper.md>).
...
1. **Why are Listeners necessary?**
    ...
    NLWeb relies on a distributed cache of [Graph 🕸 helper domains](<03 🕸🛠️ Graph helper.md>), which allows Manifest-owners to go offline without impacting whoever needs the information contained in the [Manifest 📜](<01 📜 Domain Manifest.md>). 
...
1. **What is contained in a Manifest-changed event?**
    ...
    An event from a [domain 👥](<../41 📨 Comms/00 👥 Domain.md>) about a [Manifest 📜](<01 📜 Domain Manifest.md>) change comes inside an [envelope 📨](<../41 📨 Comms/01 📨 Domain Message.md>) containing the following properties.
...
1. **How can domains know that their updates were rejected?**
    ...
    * Interested [domains 👥](<../41 📨 Comms/00 👥 Domain.md>) should [subscribe 🔔](<../42 🌬️ Streams/04 🔔🎭 Subscriber role.md>) to that [Stream 🌬️](<../42 🌬️ Streams/02 🌬️🎭 Streamer role.md>), filtering the domains they're interested in receiving alerts about.
...
1. **Do Listeners read domain Manifests from the domain?**
    ...
    * However, [domains 👥](<../41 📨 Comms/00 👥 Domain.md>) may explicitly request [Listener 👂 domains](<02 👂🛠️ Listener helper.md>) to reset the domain's [Manifest 📜](<01 📜 Domain Manifest.md>) based on a content located in a given URL, as long as the content doesn't reach a maximum size for a [Manifest 📜](<01 📜 Domain Manifest.md>).
...
1. **Why is there a size limitation on the entire Manifest?**
    ...
    Domains may ask [Graph 🕸 domains](<03 🕸🛠️ Graph helper.md>) and [Listener 👂 domains](<02 👂🛠️ Listener helper.md>) to download their [Manifest 📜](<01 📜 Domain Manifest.md>) for drift detection and sync reset. 
...
1. **How do Listeners avoid infinite loop cycles?**
    ...
    [Listener 👂 domains](<02 👂🛠️ Listener helper.md>) propagate the correlation ID sent by the original [domain 👥](<../41 📨 Comms/00 👥 Domain.md>), discarding any repeated update notifications.
