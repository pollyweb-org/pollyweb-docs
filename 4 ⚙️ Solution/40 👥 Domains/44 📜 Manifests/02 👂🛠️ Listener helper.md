...
1. **What is a Listener domain in NLWeb?**
    ...
    * with a [Streamer 🌬️ domain role](<../42 Events/02 🌬️🎭 Streamer role.md>) 
...
1. **How do Listeners work?**
    ...
    |#|Step
    |-|-
    |1| [Domains 👥](<00 👥 Domain.md>) publish their [Manifest 📜](<01 📜 Domain Manifest.md>) updates to the DNS endpoint of the global Listeners 👂 cluster. As a fallback, the same updates may be sent to an individual [Listener 👂 domain](<02 👂🛠️ Listener helper.md>).
    ...
    |4| Each [Listener 👂 domain](<02 👂🛠️ Listener helper.md>) performs a fan-out propagation of update events to all [Graph 🕸 helper domains](<03 🕸🛠️ Graph helper.md>) that [subscribed 🔔](<../42 Events/04 🔔🎭 Subscriber role.md>) to the Listener's 👂 [domain-event Stream 🌬️](<../42 Events/02 🌬️🎭 Streamer role.md>).
...
1. **How can domains know that their updates were rejected?**
    ...
    * Interested [domains 👥](<00 👥 Domain.md>) should [subscribe 🔔](<../42 Events/04 🔔🎭 Subscriber role.md>) to that [Stream 🌬️](<../42 Events/02 🌬️🎭 Streamer role.md>), filtering the domains they're interested in receiving alerts about.
    * For privacy reasons, some alerts are only be pushed to the [Subscriber 🔔 domain](<../42 Events/04 🔔🎭 Subscriber role.md>) that is referenced in the alert.
...
1. **Do Listeners propagate all notifications downstream?**
    ...
    Not necessarily. 
    - [Subscriber 🔔 domains](<../42 Events/04 🔔🎭 Subscriber role.md>) can filter the events they subscribe to.
...
1. **What if a subscriber wants to read all history?**
    ...
    [Subscriber 🔔 domains](<../42 Events/04 🔔🎭 Subscriber role.md>) can ask [Listener 👂 domains](<02 👂🛠️ Listener helper.md>) to replay all domain updates in a given period, or from the beginning of times.
...
1. **How can a subscriber filter notifications by content?**
    ...
    [Subscriber 🔔 domains](<../42 Events/04 🔔🎭 Subscriber role.md>) can set a filter when subscribing to [Listener 👂 domains](<02 👂🛠️ Listener helper.md>):
