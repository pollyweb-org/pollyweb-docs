...
1. **Why are Messages in JSON while Manifests are in YAML?** 
    ...
    | `YAML` | Structured YAML for human-to-machine settings, because it supports comments and it's easier for humans to read, while still supporting schema validations; <br/>• e.g.: [Schema Codes 🧩](<../../25 Data/24 🗄️ Vaults/02 🧩 Schema Code.md>) and [domain Manifests 📜](<../44 📜 Manifests/01 📜 Domain Manifest.md>).
...
1. **How can senders know if receivers discarded messages?**
    ...
    * Sender domains define their [Buffer ⏳ helper domain](<../../45 Helpers/10 ⏳ Buffers/03 ⏳🛠️ Buffer helper.md>) in the `Identity` section of their [domain Manifest 📜](<../44 📜 Manifests/01 📜 Domain Manifest.md>).
       * If the `Feedback` property is not defined, then no feedback is given.
       * Domains get of the [Buffer ⏳ helper domain](<../../45 Helpers/10 ⏳ Buffers/03 ⏳🛠️ Buffer helper.md>) by calling the [Identity@Broker 🚀 request](<../../../6 🅰️ APIs/45 🕸🅰️ Graph/04 👥🚀🕸 Identity.md>).
    ...
    * The feedback is sent via a [Buffer ⏳ helper domain](<../../45 Helpers/10 ⏳ Buffers/03 ⏳🛠️ Buffer helper.md>) defined by the sender's domain, using the [Feedback@Buffer 🐌 API message](<../../../6 🅰️ APIs/20 ⏳🅰️ Buffer/01 👥🐌⏳ Feedback.md>). 
        ```yaml
        🤝: nlweb.org/MSG:1.0
        Header:
