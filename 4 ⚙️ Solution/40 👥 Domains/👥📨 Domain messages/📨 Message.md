...
1. **What does a domain message look like?**
    ...
    * The [Schema 🧩](<../../30 Data/🧩 Schema Codes/🧩 Schema Code.md>) is defined at [`nlweb.org/MSG 🧩`](<📨🧩 Message schemas/🧩 MSG.md>).
...
1. **What is contained in a domain message envelope?**
    ...
    |-|-
    | `🤝` | The versioned [Schema Code 🧩](<../../30 Data/🧩 Schema Codes/🧩 Schema Code.md>) of the envelope.
...
1. **How do receiver domains handle upgraded schema versions?**
    ...
    An NLWeb envelop contains a [Schema Code 🧩](<../../30 Data/🧩 Schema Codes/🧩 Schema Code.md>) that allows receivers to support multiple versions concurrently, handling incoming envelopes differently depending on its version;
...
1. **How do receiver domains differentiate methods?**
    ...
    - e.g., `Hello@Host` refers to the [Hello 🐌 method](<../../41 🎭 Domain Roles/30 🤗 Hosts/🤗🅰️ Host methods/🤵🐌🤗 Hello.md>) in the [Host 🤗 domain role](<../../41 🎭 Domain Roles/30 🤗 Hosts/🤗🎭 Host role.md>).
...
1. **What is the signature for?**
    ...
    - and [Broker 🤵 domains](<../../20 🧑‍🦰 UI/03 🤵 Brokers/🤵🤲 Broker helper.md>) verify incoming [Wallet 🧑‍🦰 app](<../../20 🧑‍🦰 UI/01 🧑‍🦰 Wallets/🧑‍🦰 Wallet app.md>) messages with the their pre-shared public key.
...
1. **Why are Messages in JSON while Manifests are in YAML?** 
    ...
    | `YAML` | Structured YAML for human-to-machine settings, because it supports comments and it's easier for humans to read, while still supporting schema validations; <br/>• e.g.: [Schema Codes 🧩](<../../30 Data/🧩 Schema Codes/🧩 Schema Code.md>) and [domain Manifests 📜](<../👥📜 Domain Manifests/📜 Manifest.md>).
...
1. **How can senders know if receivers discarded messages?**
    ...
    * The feedback is sent via a [Buffer ⏳ helper domain](<../../45 🤲 Helper domains/27 ⏳ Buffers/⏳🤲 Buffer helper.md>) defined by the sender's domain, using the [Feedback@Buffer 🐌 API message](<../../45 🤲 Helper domains/27 ⏳ Buffers/⏳🅰️ Buffer methods/👥🐌⏳ Feedback.md>). 
        ```yaml
        🤝: nlweb.org/MSG:1.0
        Header:
