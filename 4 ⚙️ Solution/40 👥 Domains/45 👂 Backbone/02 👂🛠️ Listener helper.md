...
1. **Why are Listeners necessary?**
    ...
    NLWeb relies on a distributed cache of [Graph 🕸 helper domains](<03 🕸🛠️ Graph helper.md>), which allows Manifest-owners to go offline without impacting whoever needs the information contained in the [Manifest 📜](<../44 📜 Manifests/01 📜 Domain Manifest.md>). 
...
1. **Why is there a size limitation on the entire Manifest?**
    ...
    Domains may ask [Graph 🕸 domains](<03 🕸🛠️ Graph helper.md>) and [Listener 👂 domains](<02 👂🛠️ Listener helper.md>) to download their [Manifest 📜](<../44 📜 Manifests/01 📜 Domain Manifest.md>) for drift detection and sync reset. 
