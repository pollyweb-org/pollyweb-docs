...
1. **How do domains avoid interacting with bad actors?**
    ...
    * [sender domains 📤](<../41 📨 Messages/00 👥 Domain.md>) to know if they can [trust 👍](<01 👍 Domain Trust.md>) a [receiver domain 📥](<../41 📨 Messages/00 👥 Domain.md>) with a specific [Schema Code 🧩](<../../25 Data/24 🗄️ Vaults/02 🧩 Schema Code.md>) path - e.g. `nlweb.org/PERSONA/*`, and vice-versa.
...
1. **How to define Trust relationships?**
    ...
    * are defined in [domain Manifests 📜](<../44 📜 Manifests/$ 📜 Domain Manifest.md>) 
    * using the schema code [nlweb.org/MANIFEST/TRUST 🧩](<../44 📜 Manifests/50 🧩 TRUST code.md>).
...
1. **How to trust an unknown domain trusted by others?**
    ...
    A [domain 👥](<../41 📨 Messages/00 👥 Domain.md>) may trust an unknown domain on a specific subject if there’s a [trust 👍](<01 👍 Domain Trust.md>) third domain advocating on behalf of the unknown one;   
...
1. **How are immediate security bans imposed?**
    ...
    *  [domains 👥](<../41 📨 Messages/00 👥 Domain.md>)s may inherit all revokes from [Firewall 🔥 domains](<../../45 🛠️ Helper domains/21 🔥 Firewalls/$ 🔥🛠️ Firewall helper.md>) to immediately block any new threat, regardless of any direct or indirect [trust 👍](<01 👍 Domain Trust.md>) to the harmful actor;
...
1. **Why not using the PKI (rfc5280) to handle domain trust?**
    ...
    * partial [Trusts 👍](<01 👍 Domain Trust.md>) by subject - e.g., 
        - `A` trusts `B` for `X` but not `Y`; 
    * asymmetric [Trusts 👍](<01 👍 Domain Trust.md>) by subject - e.g., 
        - `A` trusts `B` for `X` but not `Y`, 
...
1. **How to calculate indirect trust paths when nodes are down?**
    ...
    Domains can rely on [Graph 🕸 domains](<../../42 Backbone/20 🕸 Graphs/$ 🕸🛠️ Graph helper.md>):
...
1. **How can domains know when another domain lost trust in them?**
    ...
    [Domains 👥](<../41 📨 Messages/00 👥 Domain.md>) can rely on [Graph 🕸 domain helpers](<../../42 Backbone/20 🕸 Graphs/$ 🕸🛠️ Graph helper.md>):
    - these can detect trust removals and notify affected [subscriber 🔔 domains](<../../41 🎭 Domain Roles/76 🔔 Subscribers/04 🔔🎭 Subscriber role.md>);
...
1. **How can domains know when indirect trust changes affect them?**
    ...
    [Domains 👥](<../41 📨 Messages/00 👥 Domain.md>) can rely on [Graph 🕸 domains](<../../42 Backbone/20 🕸 Graphs/$ 🕸🛠️ Graph helper.md>):
    ...
    ```
    `any-graph.com` may monitor changes in the existing [Trust 👍](<01 👍 Domain Trust.md>) path between the two banks, checking for changes like:
    * `nation-1.org` removing a [Trust 👍](<01 👍 Domain Trust.md>)  to `bank-of-nation-1.com`, or
    * `nation-1.org` removing a [Trust 👍](<01 👍 Domain Trust.md>) to `nation-2.org`.
