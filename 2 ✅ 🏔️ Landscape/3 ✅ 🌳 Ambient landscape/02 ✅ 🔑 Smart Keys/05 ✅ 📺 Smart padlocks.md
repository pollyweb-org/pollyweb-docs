📺 How safe are smart padlocks?
---

In 2023, LockPickingLawyer (a U.S.-based expert providing insights into lock vulnerabilities) published the following video, titled *"Hi-Tech Lock w/ Lo-Tech Flaw: Laxre Smart Padlock"*.

- The video demonstrates that the Laxre Smart Padlock, despite its advanced features, can be easily unlocked using a simple shim. 
- This vulnerability highlights the padlock's susceptibility to basic bypass techniques, undermining its effectiveness as a secure locking device.

<!-- https://youtu.be/xmbgDxBCeDE --><br/>

https://github.com/user-attachments/assets/938e0cc3-e04e-41f2-b492-e7f9133a8583

[405775630-938e0cc3-e04e-41f2-b492-e7f9133a8583.webm](https://github.com/user-attachments/assets/cb8ababd-ca9b-4dc6-bc23-86fb51b32597)


---




<!-- 1. Smart Padlock alternative reviews

    2023
    Unbox Daily
    published the following video, titled *"2023's Must-Have Smart Padlocks: Reviews and Recommendations"

    https://youtu.be/NOr0r0vBZbk
    https://github.com/user-attachments/assets/c12bc774-7a13-4505-8a91-bc5b9076ca71
    
    

    >>> lots of buzz words, no actual juice (noticed when only hearing the sound)


    -->
