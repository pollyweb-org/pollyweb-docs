...
1. **What's a RETURN flow?**
    ...
    * and returns one of the following expressions.
    ...
    |-|-
    |`<string>`| `3` `Alice`
    [`{Function}`](<../30 💾 Talker Data/12 🐍 {Function}.md>) | `{$placeholder}` `{handler(1)}` `{.helper(1)}` 
...
1. **What's the RETURN syntax?**
    ...
    | Argument| Purpose
    |-|-
    | `{expression}`| Optional string or [{Function}](<../30 💾 Talker Data/12 🐍 {Function}.md>) to be evaluated.
