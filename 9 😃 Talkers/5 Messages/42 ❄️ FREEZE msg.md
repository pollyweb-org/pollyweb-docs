...
1. **What is a FREEZE command?**
    ...
    A `FREEZE` ❄️
    * is a flow [Command ⌘](<../03 Talker Flows/10 ⌘ Command.md>) 
    * that freezes all previous [non-blocking Prompts 🤔](<../3 Prompts/10 🤔📘 Prompt definitions/08 ⚠️ as Status.md>)
...
1. **What's an example for FREEZE?**
    ...
    | 1️⃣ [`ONE`](<../3 Prompts/30 ✏️🤔 Input prompts/53 1️⃣ ONE prompt.md>) | Select an option, the day in this case.
    | 💼 [`SHARE`](<45 💼 SHARE msg.md>) | Get the user's booking contacts.
    | 👍 [`CONFIRM`](<../3 Prompts/30 ✏️🤔 Input prompts/31 👍 CONFIRM prompt.md>) | Pause to allow changing previous inputs.
    | ⬇️ [`EVAL`](<../03 Talker Data/20 ⬇️ EVAL flow.md>) | Save the booking.
