How to walk in into a full restaurant without a reservation?
---

> From [Eat at restaurants 🍽️](<01 🍽️ Index.md>)

## 💬 Chat

| [Domain](<../../../4 ⚙️ Solution/40 👥 Domains/👥 Domains/👥 Domain.md>) | [Prompt](<../../../4 ⚙️ Solution/35 Chats/20 🤔 Prompts/🤔 Prompt.md>) | [User](<../../../4 ⚙️ Solution/20 🧑‍🦰 UI/01 🧑‍🦰 Wallets/🧑‍🦰 Wallet app.md>)
| - | - | - |
| | | 🔆 [tap](<../../../4 ⚙️ Solution/25 Locators/15 🔆 Locators/🔆 Locator.md>) door
| 🔎 [Finder](<../../../4 ⚙️ Solution/50 🫥 Agent domains/40 🔎 Finders/🔎🫥 Finder agent.md>) | ⓘ Any Restaurant (4.3 ⭐)  [+]
| 🍽️ Restaurant | 😃 What do you need? <br/>- [ A table for a meal ] <br/>- [ Something else ] | > A table (...)
| 🍽️ Restaurant | 😃 For how many? [1, 2, +] | > +
| 🍽️ Restaurant | 😃 How many exactly? | ↕️ 5
| 🍽️ Restaurant | ❌ Sorry, not enough seats. 
| 🔎 [Finder](<../../../4 ⚙️ Solution/50 🫥 Agent domains/40 🔎 Finders/🔎🫥 Finder agent.md>) | ⓘ Here are  alternatives: <br/>- [ Betsy's (2 min) ] <br/>- [ The Guild (6 min) ] <br/>- [ Flower Pot (9 min) ] 
||

